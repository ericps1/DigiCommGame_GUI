...
log_name = [uname, '_DigiCommGame_GUI','.log'];
...
DataRate = handles.data_rate_trial; %input('What data rate would you like to attempt? ')
...
elseif (BER >= BER_req)
    set(handles.output_messages,'String',[get(handles.output_messages,'String'); strcat({'    You will need to reduce the modulation order or data rate to meet the required BER.'});]);
else
...
fprintf(log, ['Attempt ', num2str(UserTries), ': Requested Hint\n']);
fclose(log);
...
UserTries = UserTries + 1;
set(handles.UserTries,'String',UserTries);
modulation = handles.modulation_chosen; %input('Choose a Modulation Scheme ','s')
DataRate = handles.data_rate_trial; %input('What data rate would you like to attempt? ')

% write to log
log = fopen(log_name, 'a+');
fprintf(log, ['Attempt ', num2str(UserTries), ': Attempted Solution (', modulation, ' ', num2str(DataRate), ' bits/sec)\n']);
fclose(log);
...
% write to log
log = fopen(log_name, 'a+');
fprintf(log, ['User beginning level ', num2str(handles.game_level), '\n']);
fclose(log);
